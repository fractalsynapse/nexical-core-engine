...
class TeamDocumentCollectorCommandMixin(CommandMixin("team_document_collector")):
    ...
    def save_document_collection(self, event, portal_name):
        ...
        for document_hash in document_collection.team_document.values_list(
            "hash", flat=True
        ):
            ...
            if not documents:
                ...
                document = self.save_instance(
                    ...
                    None,
                    {
                        ...
                        "text": text,
                        "sentences": (
                            self.parse_sentences(text, validate=False) if text else []
                        ),
                    },
                )
                ...
                if document.hash != file_info["hash"]:
                    document.text = self.parse_file_text(
                        portal_name, "document", file_info["id"]
                    )
                    document.sentences = (
                        self.parse_sentences(document.text, validate=False)
                        if document.text
                        else []
                    )
                    document.save()
            ...
            if not documents:
                document = self.save_instance(
                    ...
                    None,
                    {
                        ...
                        "type": "bookmark",
                        "name": title,
                        "description": "{} From URL: {}".format(
                            bookmark_info["description"], url
                        ).strip(),
                        "hash": hash_value,
                        "text": text,
                        "sentences": (
                ...
                if self._store_document_topics(document, topic_parser):
                    self.data(
                        "Bookmark {} topics".format(self.key_color(document.id)),
                        document.topics,
                    )
                ...
                document = documents.first()
                document.name = title
                document.description = "{} From URL: {}".format(
                    bookmark_info["description"], url
                ).strip()
                document.save()
    ...
    def _remove_document_embeddings(self, document):
        qdrant = self.qdrant("team_document")
        qdrant.remove(document_id=document.id)
